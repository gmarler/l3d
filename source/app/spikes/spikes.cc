  ...
  public:
    ...
    void place_spike(void) {
      ...
      for(q=0; q<vlist->num_fixed_items; q++) {
        (*vlist)[q].reset();
        (*vlist)[q].transformed =	(*vlist)[q].transformed + displacement;
      }
    }
...
void my_pipeline::update_event() {
  ...
  for(i=0;i<numspikes;i++) {
    // dynamics
    spikes[i].move_spike();
    if(out_of_wc_bounds( (*spikes[i].vlist)[3].transformed))
      //- we use the vertex with index 3 because we know that this is the
      //- vertex which has the smallest z-component of all vertices.
      //- in general we don't know which vertex is foremost (since
      //- the object might be rotated or morphed, for instance),
      //- so in general we either need to check all vertices or
      //- we need to clip all vertices to make sure they are valid.
    {
      spikes[i].place_spike();
    }
